...
dockerize(){
  ...
  if [ -z "${mod_type}" ]; then
    ...
    echo "  [<path-to-mod>/.devops-wb/scan.sh] - scanning port/env-var in config files, then generate [Dockerfile]/[config.sh]"
  ...
  if [ $(grep 'springboot' <<< "${mod_type}"|wc -l) -gt 0 ]; then
    ...
    cp "$DIR"/mod-func/dummy-springboot-scan.sh .devops-wb/scan.sh
